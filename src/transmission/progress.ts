import type { Transfer, TransferFileStats } from '../types.ts';
...
export function mapPutioStatus(status: string): number {
  switch (status) {
    case 'IN_QUEUE':
    case 'WAITING':
    case 'PREPARING':
...
export function calculateTransmissionProgress(transfer: Transfer, fileStats: TransferFileStats) {
...
function remainingFromProgress(size: number, percentDone: number): number {

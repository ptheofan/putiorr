...
import type { AppConfig, ProfileInput } from './types.ts';
...
function resolvePath(value: string, cwd = process.cwd()): string {
  return path.resolve(cwd, value);
}

function jsonFromEnv(value: string | undefined, fallback: ProfileInput[]): ProfileInput[] {
  if (value == null || value === '') return fallback;
  try {
    return JSON.parse(value) as ProfileInput[];
  } catch (error) {

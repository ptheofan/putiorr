...
import { deleteLocalData, deleteLocalFileData, extractCategory } from '../download/paths.ts';
import { logger } from '../logger.ts';
import { PutioClient } from '../putio/client.ts';
import { calculateTransmissionProgress } from '../transmission/progress.ts';
...
type RpcArgValue = DbScalar | Array<string | number>;
type RpcArgs = Record<string, RpcArgValue>;
type PutioFactory = (token: string) => PutioClientLike;

function firstDefined<T>(...values: Array<T | undefined>): T | undefined {
  return values.find((value) => value !== undefined);
}

function generatedHash(): string {
  return crypto.randomBytes(20).toString('hex');
}

function deriveHashFromSource(source: string | undefined): string {
...
function deriveNameFromSource(source: string | undefined): string {
...
function stringArg(value: RpcArgValue, fallback = ''): string {
  return value == null || Array.isArray(value) ? fallback : String(value);
}

const READY_REMOTE_STATUSES = new Set(['COMPLETED', 'SEEDING']);

function remoteDeleteErrorMessage(errors: Error[]): string {
...
function putioTransferToStoreInput(transfer: PutioTransfer, fallback: TransferInput = {}): TransferInput {
...
export class TransferService {
  config: AppConfig;
  store: StateStore;
  putioFactory: PutioFactory;
  putioClient: PutioClientLike | undefined;
  putioToken: string | undefined;
  ...
  getProfileForRpcPath(rpcPath: string): Profile | undefined {
    return this.store.findProfileByRpcPath(rpcPath) ?? this.getDefaultProfile();
  }

  findProfileByCategory(category: string): Profile | undefined {
  ...
  resolveProfileForAdd(args: RpcArgs = {}, profile?: Profile): Profile | undefined {
    if (profile) return profile;
    const fallbackProfile = this.getDefaultProfile();
    const downloadDir = firstDefined(args.downloadDir, args['download-dir'], '');
    const category = extractCategory(fallbackProfile?.download_at ?? this.config.targetDir, stringArg(downloadDir));
    return this.findProfileByCategory(category) ?? fallbackProfile;
  }

  requireProfile(profile?: Profile): Profile {
  ...
  async ensureProfileFolder(profile?: Profile): Promise<Profile> {
    ...
    const updated = this.store.updateProfile(current.id, { putio_folder_id: folderId });
    if (!updated) throw new Error('RR profile disappeared while creating put.io folder');
    return updated;
  }

  async addTorrent(args: RpcArgs = {}, profile?: Profile) {
    const currentProfile = await this.ensureProfileFolder(this.resolveProfileForAdd(args, profile));
    const filename = stringArg(firstDefined(args.filename, args.url), '');
    const magnetLink = stringArg(firstDefined(args.magnetLink, args['magnet-link']), '');
    const metainfo = stringArg(args.metainfo, '');
    const downloadDir = firstDefined(args.downloadDir, args['download-dir'], '');
    const category = extractCategory(currentProfile.download_at, stringArg(downloadDir));

    let putioTransfer: PutioTransfer | undefined;
    let source: string;
    ...
    if (!putioTransfer) throw new Error('put.io did not return a transfer');
    ...
    const row = this.store.createOrUpdateTransfer(putioTransferToStoreInput(putioTransfer, {
      ...
      category,
      download_dir: stringArg(downloadDir),
      lifecycle: 'remote',
      save_parent_id: currentProfile.putio_folder_id,
    }));
    if (!row) throw new Error('Could not save transfer');
  ...
  async refreshRemoteTransfers(): Promise<Transfer[]> {
    ...
    for (const remote of remoteTransfers) {
      ...
      const row = this.store.createOrUpdateTransfer(putioTransferToStoreInput(remote, {
  ...
  pruneRemovedTransfers(remoteIds: Set<number>, remoteHashes: Set<string>): void {
  ...
  async getTorrents(args: RpcArgs = {}, profile?: Profile) {
    ...
    const rows = requestedIds.length > 0
      ? requestedIds.map((id) => this.store.findTransfer(id)).filter((row) => row !== undefined)
      : this.store.listActiveTransfers({ profileId: profile?.id });

    const torrents = rows.map((row) => this.toTransmissionTorrent(row, fields.map(String)));
    return { torrents };
  }

  async removeTorrents(args: RpcArgs = {}, profile?: Profile) {
    ...
    for (const id of ids) {
      ...
      if (deleteLocal) {
        ...
        if (!transferProfile) throw new Error('No RR profile is available for download');
  ...
  async deleteDownloadBucket(
    transferId: number,
    { deleteRemote = true, deleteLocal = true }: { deleteRemote?: boolean; deleteLocal?: boolean } = {},
  ) {
  ...
  refreshTransferAfterFileDeletion(transferId: number): Transfer | undefined {
  ...
  async removeRemoteFiles(files: TransferFile[], { throwOnError = false }: { throwOnError?: boolean } = {}): Promise<Error[]> {
    const errors: Error[] = [];
    const putio = this.getPutio();
    for (const file of files) {
      try {
  ...
  async removeRemoteTransfer(transfer: Transfer, { throwOnError = false }: { throwOnError?: boolean } = {}): Promise<Error[]> {
    const errors: Error[] = [];
    const putio = this.getPutio();
  ...
  toTransmissionTorrent(row: Transfer, requestedFields: string[] = []) {
    ...
    const filtered: Record<string, object | string | number | boolean | string[]> = {};
    const torrentRecord = torrent as Record<string, object | string | number | boolean | string[]>;
    for (const field of requestedFields) {
      if (Object.hasOwn(torrent, field)) {
        filtered[field] = torrentRecord[field];
      }
    }
  ...
  toTransmissionFile(row: Transfer, file: TransferFile) {
  ...
  toTransmissionFileStats(row: Transfer, file: TransferFile) {
    return {
  ...
  calculateFileRpcProgress(row: Transfer, file: TransferFile): number {
    const size = Number(file.size ?? 0);

...
import { downloadPolicyForContext, isSlowSpeedResetEnabled } from './policy.ts';
import { fileExistsWithSize, normalizeRelativePath, resolveInside } from './paths.ts';
import type { StateStore } from '../state/store.ts';
import type { TransferService } from '../transfer/service.ts';
import type { AppConfig, Profile, Transfer, TransferFile } from '../types.ts';
...
export class DownloadManager {
  ...
  activeFileRates: Map<number, { transferId: number; bytesPerSecond: number }>;
  ...
  async prepareTransferSafely(row: Transfer): Promise<void> {
    try {
  ...
  async startTransferDownload(transferId: number) {
  ...
  recordTransferStartFailure(transfer: Transfer | undefined, error: Error & { status?: number }, message: string): void {
    if (transfer?.id) {
  ...
  async prepareTransfer(transfer: Transfer): Promise<void> {
    ...
    if (!updated) throw new Error('transfer disappeared while preparing download');
    let totalSize = 0;
    for (const remoteFile of remoteFiles) {
      if (remoteFile.id == null) throw new Error('put.io file id is missing');
  ...
  async pruneProcessedTransfersMissingLocalData(): Promise<void> {
  ...
  async hasLocalTransferData(profile: Profile, transfer: Transfer): Promise<boolean> {
    const files = this.store.listFilesForTransfer(transfer.id);
    if (files.length === 0) {
  ...
  async workerLoop(index: number): Promise<void> {
  ...
  nextPendingFile(): TransferFile | undefined {
  ...
  async processFile(file: TransferFile): Promise<void> {
  ...
  isFileDeletedOrTransferRemoved(file: TransferFile): boolean {
    const transfer = this.store.findTransferById(file.transfer_id);
    if (!transfer || transfer.removed_at) return true;
    return this.store.findTransferFileById(file.id)?.status === 'deleted';
  }

  async discardLocalFile(targetPath: string): Promise<void> {
  ...
  async downloadToPath(downloadUrl: string, targetPath: string, file: TransferFile): Promise<void> {
  ...
  async downloadAttempt(
    ...
    file: TransferFile,
    resetCount: number,
  ): Promise<{ complete?: boolean; reset?: boolean }> {
  ...
  linkAbortSignal(sourceSignal: AbortSignal, targetController: AbortController): () => void {
  ...
  createSlowSpeedGuard(file: TransferFile, controller: AbortController, initialBytes: number, resetCount: number) {
  ...
  downloadPolicyForFile(file: TransferFile) {
    const transfer = this.store.findTransferById(file.transfer_id);
  ...
  async updateAfterSlowReset(file: TransferFile, partPath: string, message: string, resetCount: number): Promise<void> {
  ...
  updateLocalProgressMetrics(file: TransferFile, downloadedBytes: number, bytesPerSecond: number): void {
  ...
  refreshTransferLocalMetrics(transferId: number): void {
  ...
  async finalizeTransferIfComplete(transferId: number): Promise<void> {
    const transfer = this.store.findTransferById(transferId);

...
function run(
  command: string,
  args: string[],
  { allowFailure = false, capture = false, env = {} }: RunOptions = {},
...
function packageJsonPath(): string | URL {
...
function writePackageVersion(version: string): boolean {
  const packageJson = readPackageJson();
...
function checkCleanGit({ allowDirty }: Pick<ReleaseOptions, 'allowDirty'>): void {
...
function checkMain({ allowNonMain }: Pick<ReleaseOptions, 'allowNonMain'>): void {
...
function checkUpToDate({ allowUnpushed }: Pick<ReleaseOptions, 'allowUnpushed'>): string {
...
function resolveRepo(explicitRepo?: string): string {
  ...
  return String((JSON.parse(result) as { nameWithOwner?: string }).nameWithOwner ?? '');
}

function checkReleaseDoesNotExist({ repo, tag }: { repo: string; tag: string }): void {
...
function revertMetadataIfChanged(): void {
